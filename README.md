...
3. **Add tab** — capture a phrase by typing, or by picking a line out of a recording's transcript, sharing one mode toggle at the top: **Hebrew phrase** (the default — typos corrected, translated into English) or **Check my English** (input is already English, or mixed English/Hebrew — corrected for grammar and phrasing instead of translated). Type and hit Send, and it's saved right away into the shared log — the same AI call also tries to match it to one of the space's existing tags, only when confident (otherwise it's left untagged, and you can always tag or retag it yourself from Practice). No confirmation step, by design. Picking an audio recording from your phone (up to ~30 minutes) only transcribes it — nothing is auto-translated or auto-tagged from it. A small 📄 button swaps the window over to a list of past recording transcripts (tap one to expand/collapse, delete once you're done with it; a nudge appears once more than 3 are saved); selecting text inside an expanded transcript surfaces an "Add to input" bar — tap it to drop that exact selection into the input, edit if needed, then Send, same as typing it fresh. Repeat per phrase; tap 📄 again (now ✏️) to go back to capturing directly
...
- **The mode toggle defaults from `phrases.mode`.** Every phrase now remembers which mode (Hebrew phrase / Check my English) it was created with, and editing defaults the toggle to that. Older phrases with `NULL` there default to Hebrew phrase mode — switch the toggle manually first if that's wrong for a specific one.
...
- **Selecting from a transcript.** Opening the 📄 transcript list and selecting text inside an expanded transcript body (the phone's normal long-press selection) surfaces an "Add to input" bar; tapping it drops that exact selection into the Add tab's input, switching back to the normal capture view so it can be edited and sent like any typed phrase. Repeat per phrase.
